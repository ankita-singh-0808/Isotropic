...
plt.scatter(y, y_pred, alpha=0.6, color='green', label='Cross-Validated Predictions')
plt.plot([y.min(), y.max()], [y.min(), y.max()], 'r--', label='Ideal Fit')
plt.title(f'RF: Actual vs Predicted\nR² = {overall_r2:.4f}')
...
# R² bar plot
plt.subplot(1, 2, 1)
sns.barplot(x=fold_labels, y=r2_scores, hue=fold_labels, palette='Greens_r', legend=False)
plt.axhline(np.mean(r2_scores), color='red', linestyle='--', label=f'Mean R² = {np.mean(r2_scores):.4f}')
plt.title('RF: R² Score per Fold')
plt.xlabel('Fold')
plt.ylabel('R² Score')
plt.ylim(-1, 1)
plt.legend()
...
# MSE bar plot
plt.subplot(1, 2, 2)
sns.barplot(x=fold_labels, y=mse_scores, hue=fold_labels, palette='Blues_r', legend=False)
plt.axhline(np.mean(mse_scores), color='red', linestyle='--', label=f'Mean MSE = {np.mean(mse_scores):.4f}')
plt.title('RF: MSE per Fold')
plt.xlabel('Fold')
...
plt.title('Feature Sensitivity Analysis (±10%) - Random Forest')
