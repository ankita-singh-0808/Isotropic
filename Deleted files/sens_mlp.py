...
# Define pipeline: Scaling + MLP Regressor
pipeline = Pipeline([
...
plt.scatter(y, y_pred, alpha=0.6, color='orange', label='Cross-Validated Predictions')
...
# R² bar plot
plt.subplot(1, 2, 1)
sns.barplot(x=fold_labels, y=r2_scores, hue=fold_labels, palette='Oranges', legend=False)
...
# MSE bar plot
plt.subplot(1, 2, 2)
sns.barplot(x=fold_labels, y=mse_scores, hue=fold_labels, palette='Reds', legend=False)
...
plt.title('Feature Sensitivity Analysis (±10%) - MLP')
