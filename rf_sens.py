...
# Train-test split for separate plots
...
# -------------------------------
# ✅ Linear Sensitivity Grid Plots
# -------------------------------
def plot_linear_sensitivity_grid_fullrange(pipeline, X, feature_names, steps=50):
    X_mean = X.mean().to_frame().T

    # Filter out dummy variables (those starting with 'LoadPosition_')
    filtered_features = [f for f in feature_names if not f.startswith("LoadPosition_")]
    ...
    for i, col in enumerate(filtered_features):
        ...
        col_values = np.linspace(col_min, col_max, steps)

        X_test = pd.concat([X_mean] * steps, ignore_index=True)
        X_test[col] = col_values

        y_pred = pipeline.predict(X_test)

        axes[i].plot(col_values, y_pred, marker='o', color='forestgreen')
    ...
    plt.suptitle('Random Forest: Sensitivity Analysis', fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.show()


# Plot sensitivity curves
plot_linear_sensitivity_grid_fullrange(pipeline, X, feature_names)
